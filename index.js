...
const fetchBuffer = async (url, attempts = 5) => {
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
        try {
            ...
            if (!buffer.length) throw new Error('Сервер вернул пустой ответ')
            return buffer
        } catch (error) {
            lastError = error
...
const parseAttributeList = (line) => {
    ...
    while ((match = regex.exec(value)) !== null) {
        ...
        if (attributeValue.startsWith('"') && attributeValue.endsWith('"')) attributeValue = attributeValue.slice(1, -1)
...
const parsePlaylist = (text, playlistUrl) => {
    ...
    for (const line of lines) {
        ...
        if (line.startsWith('#EXT-X-MEDIA-SEQUENCE:')) {
            ...
            if (!Number.isSafeInteger(mediaSequence)) throw new Error(`Некорректный MEDIA-SEQUENCE: ${value}`)
            continue
        }
        ...
        if (line.startsWith('#EXT-X-KEY:')) {
            ...
            if (method !== 'AES-128') throw new Error(`Неподдерживаемый HLS METHOD: ${method}`)
            if (!attrs.URI) throw new Error('AES-128 key без URI')
            const keyFormat = attrs.KEYFORMAT || 'identity'
            if (keyFormat !== 'identity') throw new Error(`Неподдерживаемый KEYFORMAT: ${keyFormat}`)
            currentKey = {
                method: 'AES-128',
                uri: new URL(
                    attrs.URI,
        ...
        if (line.startsWith('#EXTINF:')) {
            ...
            if (!Number.isFinite(currentDuration) || currentDuration <= 0) throw new Error(`Некорректный EXTINF: ${value}`)
            continue
        }
        if (line.startsWith('#')) continue
        if (currentDuration === null) throw new Error(`Найден сегмент без EXTINF: ${line}`)
    ...
    if (!segments.length) throw new Error('В m3u8 не найдено ни одного сегмента')
    return segments
}

const keyCache = new Map()

const getHlsKey = async (url) => {
...
const makeIv = (segment) => {
    if (segment.key.iv) {
        let hex = segment.key.iv;
        if (hex.startsWith('0x') || hex.startsWith('0X')) hex = hex.slice(2);
        if (!/^[0-9a-fA-F]+$/.test(hex)) throw new Error(`Некорректный IV: ${segment.key.iv}`)
...
const concatTsSegments = async (segments, outputFile) => {
    ...
    for (const segment of segments) {
        ...
        if (!data.length) throw new Error(`Сегмент #${segment.index} пустой`)
...
const validatePcm = async (pcmFile, expectedDuration) => {
    ...
    if (!stat.size) throw new Error('FFmpeg создал пустой PCM')
...
const main = async () => {
    ...
    try {
        ...
        if (!playlistText.includes('#EXTM3U')) throw new Error('Ответ VK не похож на HLS playlist')
        ...
        if (!outputStat.size) throw new Error('Финальный MP3 пустой')
        success = true
    } catch (error) {
        console.error(error)
    } finally {
